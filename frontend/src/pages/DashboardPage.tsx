...
export default function DashboardPage() {
  ...
  const calendarOnDashboard = dashboardWidgets.some((w) => w.widgetId === 'calendar')
  const availableCalendar = WIDGETS.find((w) => w.id === 'calendar')
    ...
    <div className="flex-1 p-8 overflow-y-auto" ref={containerRef}>
      ...
      >
        {dashboardWidgets.map((instance) => {
          ...
          const isCalendar = instance.widgetId === 'calendar'
          const isWeather = instance.widgetId === 'weather'

          // Weather widget - special green background
          if (isWeather) {
          ...
          // Calendar widget - with settings + remove
          if (isCalendar) {
            return (
              <div
              ...
              >
                ...
                {/* Header */}
                <div className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#b5b5a8' }}>
                  {config.name}
                </div>
                {/* Calendar with settings */}
                <CalendarWidget
          ...
          // Other widgets (todo, schedule) — standard rendering
          return (
            <div
              key={instance.instanceId}
              className="rounded-2xl p-5 relative group"
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              ...
              {availableCalendar && canSeeWidget(availableCalendar.requiredRole, user?.role) && (
                <div
                  className="flex items-center gap-3 p-3 rounded-xl"
                  style={{
                    background: calendarOnDashboard ? '#f8f8f4' : '#ffffff',
                    border: '1px solid #e8e8e2',
                    opacity: calendarOnDashboard ? 0.6 : 1,
                  }}
                >
                  <div className="text-2xl">📅</div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium" style={{ color: '#2d2d2d' }}>
                      {availableCalendar.name}
                    </div>
                    <div className="text-xs" style={{ color: '#9e9e96' }}>
                      {availableCalendar.description}
                    </div>
                  </div>
                  {calendarOnDashboard ? (
              ...
              {/* Other widgets - shown as info */}
              {WIDGETS.filter((w) => w.id !== 'calendar').map((w) => {
                const isActive = dashboardWidgets.some((dw) => dw.widgetId === w.id)
                const emoji = w.id === 'weather' ? '🌤️' : w.id === 'todo' ? '✅' : '📋'
                return (
                  <div
                    key={w.id}
                    className="flex items-center gap-3 p-3 rounded-xl"
                    style={{ background: '#f8f8f4', border: '1px solid #e8e8e2', opacity: 0.5 }}
                  >
                    <div className="text-2xl">{emoji}</div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium" style={{ color: '#2d2d2d' }}>{w.name}</div>
                      <div className="text-xs" style={{ color: '#9e9e96' }}>{w.description}</div>
                    </div>
                    <span className="text-xs px-2 py-1 rounded-lg" style={{ background: '#f0f5f0', color: isActive ? '#7c9a7e' : '#9e9e96' }}>
                      {isActive ? 'Aktiv' : 'Demnächst'}
                    </span>
                  </div>
                )
              })}
