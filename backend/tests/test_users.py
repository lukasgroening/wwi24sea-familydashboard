...
def test_get_all_users_regular_user_forbidden(client, test_user):
    ...
    assert response.json()["detail"] == "Zugriff verweigert. Nur Administratoren dürfen Benutzer verwalten."
...
def test_delete_self_forbidden(client, admin_user):
    ...
    assert response.json()["detail"] == "Du kannst deinen eigenen Account nicht löschen."
